...
export default class KlanjeScreen extends Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    getData(data.production.klanje).then(data => this.setState({ data }));
  }

  handleRefresh() {
    this.setState({
      refreshing: true
    }, ()=> getData(data.production.klanje).then(data => this.setState({ data, refreshing: false })))
  }
