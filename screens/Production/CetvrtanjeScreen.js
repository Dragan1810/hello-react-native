...
export default class CetvrtanjeScreen extends Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    getData(data.production.cetvrtanje).then(data => this.setState({ data }));
  }

  handleRefresh() {
    this.setState({
      refreshing: true
    }, ()=> getData(data.production.cetvrtanje).then(data => this.setState({ data, refreshing: false })))
  }
